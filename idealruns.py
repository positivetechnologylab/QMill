...
SIM_DATA_DIR  = "ce_high_sim_data_ideal"
SHOTS         = 2048
...
np.savez(
    os.path.join(SIM_DATA_DIR, "ce_swaptest_high_data_ideal.npz"),
    phi_soil=ps_arr, phi_free=pf_arr,
    p0=p0_arr, ce=ce_arr
)

with open(os.path.join(SIM_DATA_DIR, "ce_swaptest_high_data_ideal.csv"),
          "w", newline="") as f:
    writer = csv.writer(f)
...
print("\nIdeal (noise‑free) simulation results saved in:", SIM_DATA_DIR)